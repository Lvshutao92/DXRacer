...
#define itemHeigth     44  //每项高度
#define triangleWith   15  //三角形的高度

#define itemSelectColor  [UIColor lightGrayColor]
...
- (id)initWithFrame:(CGRect)frame items:(NSArray<NSString *> *)items showPoint:(CGPoint)showPoint{
    ...
    if (self) {
        ...
        if (self.items.count != 0) {
            ...
            [self.myTabelView selectRowAtIndexPath:ip animated:YES scrollPosition:UITableViewScrollPositionBottom];
        }
...
- (UITableView *)myTabelView{
    ...
    _myTabelView.backgroundColor = [[UIColor blackColor]colorWithAlphaComponent:0.1];
    _myTabelView.scrollEnabled = YES;
...

{
    ...
    UIColor * clor = [[UIColor blackColor]colorWithAlphaComponent:0.5];
    [clor setFill];  //设置填充色
